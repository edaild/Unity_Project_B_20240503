...
public class UIManager : MonoBehaviour
{
     ...
     void OnEnable()
    {
        ...
        GameManager.OnBestScorehanged += UpdateBestScoreText;      // 이벤트 등록
    }

    void OnDisable()
    {
        GameManager.OnPointChanged -= UpdatePointText;      //이벤트 삭제
        GameManager.OnBestScorehanged -= UpdateBestScoreText;  //이벤트 삭제
    }

     void UpdatePointText(int newPoint)             //함수 이벤트를 만들어서 인수로 설정
    {
        pointText.text = "points : " + newPoint; 
    }
    
    void UpdateBestScoreText(int newBestScore)      // 함수 이벤트를 만들어서 인수로 설정
    {
